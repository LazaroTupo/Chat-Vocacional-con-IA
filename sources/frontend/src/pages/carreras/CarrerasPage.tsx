...
export default function CarrerasPage() {
  ...
  const loadRecommendations = async () => {
    ...
    try {
      ...
      if (response.careers && response.careers.length > 0) {
        ...
        // Guardar también en localStorage para la página de universidades
        ...
        setError("No se encontraron carreras recomendadas para tu perfil.");
      }
    } catch (err) {
  ...
  useEffect(() => {
    const savedCareers = localStorage.getItem('careerRecommendations');
    if (savedCareers) {
      ...
      <div className="py-10 text-center">
        ...
        <p className="py-4">
          {hasLoaded 
            ...
            : "Descubre las carreras que mejor se adaptan a tu vocación"
          }
        </p>
            ...
            >
              {isLoading ? (
                ...
                <>
                  ...
                  Cargar carreras según tu historial
                </>
              )}
            </button>
          </div>
        ...
        {hasLoaded && (
          <div className="mb-6">
            <p className="text-sm text-gray-600 mb-3">
              ¿Quieres ver nuevas carreras según tu historial actualizado?
            </p>
            <button
              onClick={loadRecommendations}
              disabled={isLoading}
      ...
      {hasLoaded ? (
        careers.length > 0 ? (
          ...
          <div className="text-center py-10">
            <p className="text-gray-500">No se encontraron carreras recomendadas.</p>
          </div>
        )
      ) : (
        <div className="text-center py-20">
          <div className="max-w-md mx-auto">
            <i className="fa-solid fa-graduation-cap text-6xl text-gray-300 mb-4"></i>
            <h3 className="text-xl font-bold mb-2">Carreras personalizadas</h3>
            <p className="text-gray-600 mb-6">
              Haz clic en "Cargar carreras según tu historial" para descubrir las carreras que mejor se adaptan a tu perfil vocacional basado en tu conversación con el coach.
            </p>
          </div>
        </div>
      )}
